...
def generate_rich_template():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Demand (5 Years) - ASYMMETRIC
        years, regs = [1,2,3,4,5], ["London", "Scotland", "North_East", "South_West", "NI"]
        demand_matrix = [{"Year": y, "London": 18000*y, "Scotland": 7500*y, "North_East": 9000*y, "South_West": 8500*y, "NI": 3800*y} for y in years]
        pd.DataFrame(demand_matrix).to_excel(writer, sheet_name="Demand", index=False)
        # Freight - UNIQUE RATES
...
with st.expander("📌 Strategic Objective & Audit Logic", expanded=False):
    st.markdown("""
    **Objective:** Identify the network path that maximizes **5-Year NPV**.
    **Profit Max Mode:** The solver treats demand as a ceiling. It rationalizes markets by exiting regions where logistics and returns costs exceed the unit profit.
    **Auditability:** Contribution Margins are lane-specific, calculated using the specific logistics path chosen by the solver.
    """)
...
with st.sidebar:
    ...
    st.header("🏢 Strategy")
    strategy = st.radio("Distribution Asset Model", ["Optimize Mix", "3PL Only", "Owned Only"])
...
if run_button:
    ...
    for y in years:
        for r in regs:
            total_v = pulp.lpSum([f_last[(d, r, y)] for d in dcs])
            if service_mode == "Capture All Demand": model += total_v == df_dem.loc[y, r]
            else: model += total_v <= df_dem.loc[y, r]
        for d in dcs:
    ...
    for y in years:
        ...
        c_prod = pulp.lpSum([f_in[(s, f, y)] * (df_sup.loc[s, 'RM_Cost']*1.2 + df_f_in.loc[s, f]) for s in sups for f in facs]) * inf
        c_log = (pulp.lpSum([f_out[(f, d, y)] * df_f_out.loc[f, d] for f in facs for d in dcs]) + 
                 pulp.lpSum([f_last[(d, r, y)] * (df_last.loc[d, r] + df_3pl.loc[d, 'Variable_Handling_Cost']) for d in dcs for r in regs])) * inf
        c_rev = pulp.lpSum([f_last[(d, r, y)] * sim_returns * (df_last.loc[d, r] + sim_refurb) for d in dcs for r in regs]) * inf
        ...
        cfs.append((rev - c_prod - c_log - c_rev - fixed - capex) / ((1 + WACC)**y))

    model += pulp.lpSum(cfs)
    model.solve(pulp.PULP_CBC_CMD(msg=0))

    # --- 5. VISUALIZATION & AUDIT ---
    st.metric("Total Strategy NPV (5-Year)", f"£{pulp.value(model.objective)/1e6:.2f}M")
    t1, t2, t3, t4 = st.tabs(["🏗️ Build Schedule", "🚚 Volume Flow (Units)", "📍 Regional Profitability", "📥 CFO Audit Ledger"])
    ...
    with t1:
        ...
        st.table(pd.DataFrame(builds) if builds else pd.DataFrame([{"Builds": "Asset-Light Solution Chosen"}]))

    with t2:
        st.subheader("Physical Unit Flow (5-Year Density)")
            ...
            if vol > 0:
                ...
                for y in years:
                    for d in dcs:
                        ...
                        if v > 0:
                            # Forensic lane costing for this region
                            t_cost += v * (df_sup.iloc[0]['RM_Cost']*1.2 + 150 + df_last.loc[d,r] + df_3pl.loc[d,'Variable_Handling_Cost'] + sim_returns*(df_last.loc[d,r]+sim_refurb))
                reg_stats.append({"Region": r, "Units": int(vol), "Contribution Margin %": round((t_rev-t_cost)/t_rev*100, 1), "Net Profit (£)": t_rev-t_cost})
                ...
                for r in regs:
                    ...
                    if v > 0:
                        ledger.append({
                            ...
                            "Revenue": v*PRICE,
                            "Forward_Last_Mile": v*df_last.loc[d,r],
                            "DC_Handling": v*df_3pl.loc[d,'Variable_Handling_Cost'],
                            "Reverse_Logistics_Cost": v*sim_returns*(df_last.loc[d,r] + sim_refurb),
                            "Landed_Margin": v*PRICE - (v*(df_sup.iloc[0]['RM_Cost']*1.2 + 150 + df_last.loc[d,r] + df_3pl.loc[d,'Variable_Handling_Cost'] + sim_returns*(df_last.loc[d,r]+sim_refurb)))
                        })
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            pd.DataFrame(ledger).to_excel(writer, sheet_name="Path_Forensics", index=False)
            pd.DataFrame(reg_stats).to_excel(writer, sheet_name="Regional_Profit", index=False)
